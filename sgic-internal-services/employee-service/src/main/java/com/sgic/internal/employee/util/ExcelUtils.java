...
public class ExcelUtils {
  ...
  /*
   * public static ByteArrayInputStream customersToExcel(List<Employee> employees) throws
   * IOException { System.out.println("excelutils1"); String[] COLUMNs = { "employee_id", "name",
   * "email" }; try (Workbook workbook = new XSSFWorkbook(); ByteArrayOutputStream out = new
   * ByteArrayOutputStream();) { CreationHelper createHelper = workbook.getCreationHelper();
   * 
   ...
   * headerRow.createCell(col); cell.setCellValue(COLUMNs[col]); cell.setCellStyle(headerCellStyle);
   * }
   * 
   * // CellStyle for Age CellStyle ageCellStyle = workbook.createCellStyle();
   * ageCellStyle.setDataFormat(createHelper.createDataFormat().getFormat("#"));
   * 
   * int rowIdx = 1; for (Employee employee : employees) { Row row = sheet.createRow(rowIdx++);
   * 
   ...
   * 
   * // Cell ageCell = row.createCell(3); // ageCell.setCellValue(employee.getDesignation()); //
